        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @forelse ($rows as $item)
                    <tr role="button" wire:click="$emit('set:brand', '{{ $item->brandid }}')" class="bg-white cursor-pointer border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-200">
                        <th scope="row"
                            class="px-4 py-2 {{ $brandId == $item->brandid ? 'font-bold text-blue-700 dark:text-blue-200':'font-medium text-gray-900 dark:text-white' }} whitespace-nowrap">
                            {{ $item->brandname }}
                        </th>
                    </tr>
                @empty
