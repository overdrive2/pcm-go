        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @forelse ($rows as $item)
                    <tr role="button" wire:click="$emit('set:item-type', '{{ $item->id }}', '{{ $item->prefix }}')" class="cursor-pointer border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-200 bg-white">
                        <th scope="row"
                            ...
                            {!! str_replace($search, '<span class="bg-yellow-200">'.$search.'</span>', $item->iname) !!}
                        </th>
                        <td class="px-4 py-2">
                            {{ $item->prefix }}
                        </td>
