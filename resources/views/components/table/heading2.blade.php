...
<th scope="col"
    {{ $attributes->merge(['class' => 'hidden lg:table-cell px-6 py-3 font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300'])->only('class') }}
>
    @unless ($sortable)
        <span class="text-left leading-4 font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left leading-4 font-medium dark:text-gray-300 text-gray-500 uppercase tracking-wider group focus:outline-none">
            <span>{{ $slot }}</span>
